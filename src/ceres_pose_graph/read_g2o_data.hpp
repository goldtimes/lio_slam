...
static bool readG2O(const std::string &file_path, std::vector<Pose3D> &vertexs, std::vector<BinaryEdge> &binary_edges) {
    ...
    while (g2o_file.good()) {
        ...
        if (data_type == "VERTEX_SE3:QUAT") {
            ...
            g2o_file >> binary_edge.id_a >> binary_edge.id_b >> binary_edge.constraint_pose.p.x() >>
                binary_edge.constraint_pose.p.y() >> binary_edge.constraint_pose.p.z() >>
                binary_edge.constraint_pose.q.x() >> binary_edge.constraint_pose.q.y() >>
                binary_edge.constraint_pose.q.z() >> binary_edge.constraint_pose.q.w();
            for (int i = 0; i < 6; i++) {
                for (int j = i; j < 6; j++) {
